...
for workerPath in ${workerPaths[@]}; do
  ...
  if ! grep -q ${workerInstanceId} ${tmp_dir}/running-instances.txt; then
    rm -f ${workerPath}
    echo "removing ${workerPath}" >> ${HOME}/cron/log/minionsmanaged-observer/$(/usr/bin/date -u '+%Y-%m-%d').log
  fi
done
...
git --git-dir=${tmp_dir}/observations/.git --work-tree=${tmp_dir}/observations commit -m "observations of removed instances in aws" >> ${HOME}/cron/log/minionsmanaged-observer/$(/usr/bin/date -u '+%Y-%m-%d').log
